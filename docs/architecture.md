...
2. **Signal adapters** — resource property, Resource Graph, Log Analytics/Application Insights, and Azure Monitor metric implementations returning healthy, failed, or unconnectable.
...
Rule Studio -> source registry -> reviewed Azure adapter -> normalized findings
                 | property      | zero findings = healthy
                 | graph KQL     | confirmed finding = red
                 | logs KQL      | missing access/data = grey
                 ` metric        ` credentials never enter the rule
```
